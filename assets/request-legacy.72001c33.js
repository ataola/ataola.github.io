...
axios$2.exports.default=axios$1;var axios=axios$2.exports;var Request=/*#__PURE__*/function(){function Request(){_classCallCheck(this,Request);}_createClass(Request,null,[{key:"init",value:function init(){this.axiosInstance=axios.create({baseURL:"/api",timeout:1e4});this.initInterceptors();}},{key:"initInterceptors",value:function initInterceptors(){this.axiosInstance.defaults.withCredentials=true;this.axiosInstance.defaults.headers["X-Requested-With"]="XMLHttpRequest";this.axiosInstance.defaults.headers.post["Content-Type"]="application/json";this.axiosInstance.interceptors.request.use(function(config){var token=localStorage.getItem("ACCESS_TOKEN");if(token&&config&&config.headers){config.headers.Authorization="Bearer "+token;}return config;},function(error){alert(error);});this.axiosInstance.interceptors.response.use(function(response){var _response$data=response.data,code=_response$data.code,message=_response$data.message,data=_response$data.data;if(response.status!==200||code!==0){Request.errorHandle(response,message);}return data;},function(error){var response=error.response;if(response){Request.errorHandle(response);}else{Message({text:"\u7F51\u7EDC\u8FDE\u63A5\u5F02\u5E38,\u8BF7\u7A0D\u540E\u518D\u8BD5!",type:"error"});}return Promise.reject(response===null||response===void 0?void 0:response.data);});}},{key:"errorHandle",value:function errorHandle(res,message){switch(res.status){case 401:break;case 403:break;case 404:Message({text:"\u8BF7\u6C42\u7684\u8D44\u6E90\u4E0D\u5B58\u5728",type:"error"});break;default:message&&Message({text:message,type:"error"});}}}]);return Request;}();_defineProperty(Request,"axiosInstance",void 0);exports('R',Request);}};});})();
